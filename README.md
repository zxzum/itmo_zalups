...
В проекте реализован графический интерфейс на Qt6 для удобной работы с ITMO Loops.
...
Если Qt6 установлен, будут собраны оба исполняемых файла:
- `build/bin/itmoloops` - консольная версия
- `build/gui/itmoloops_gui` - GUI версия

### Запуск GUI

```bash
./build/gui/itmoloops_gui
```

### Возможности GUI

1. **Загрузка score файлов** - выбор и загрузка композиций через диалог
2. **Редактор с подсветкой синтаксиса** - просмотр и правка score файлов
3. **Информация о композиции** - таблица инструментов, список паттернов, BPM
4. **Генерация WAV** - создание аудио файлов с отображением статуса
5. **Превью волновой формы** - визуализация сгенерированного аудио
6. **Воспроизведение** - открытие в системном плеере одним кликом
7. **Тёмная тема** - современный интерфейс
8. **Русский язык** - весь интерфейс на русском

### Исправленная проблема

GUI правильно обрабатывает пути к файлам:
- `notes.txt` ищется относительно директории score файла
- Пути к sample файлам преобразуются в абсолютные
- Генерация работает независимо от текущей рабочей директории

Подробнее см. [gui/README.md](gui/README.md)
